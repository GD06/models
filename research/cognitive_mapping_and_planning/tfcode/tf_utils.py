...
def prepare_feed_dict(input_tensors, inputs):
  feed_dict = {}
  for n in list(input_tensors.keys()):
    if n in inputs.keys():
      feed_dict[input_tensors[n]] = inputs[n].astype(input_tensors[n].dtype.as_numpy_dtype)
    else:
      tensor_shape = input_tensors[n].shape
      print('Tensor name: {} and shape: {}'.format(n, tensor_shape.as_list()))
      int_shape = []
      for i in tensor_shape.as_list():
        if i is None:
          int_shape.append(int(1))
        else:
          int_shape.append(i)
      random_array = np.random.random(size=tuple(int_shape))
      feed_dict[input_tensors[n]] = random_array.astype(input_tensors[n].dtype.as_numpy_dtype)
...
def add_summary_ops(m, summarize_ops, summarize_names, to_aggregate=None,
  ...
  if np.any(to_aggregate):
    ...
    agg_values_op, agg_update_op = list(zip(*agg_ops))
    agg_update_op = [x for x in agg_update_op if x is not None]
...
def accum_val_ops(outputs, names, global_step, output_dir, metric_summary, N):
  ...
  for i in range(len(outputs[0])):
    scalar = np.array([x[i] for x in outputs])
